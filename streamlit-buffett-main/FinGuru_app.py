...
def creds_entered():
    if len(st.session_state["streamlit_username"])>0 and len(st.session_state["streamlit_password"])>0:
          ...
          elif st.session_state["streamlit_username"].strip() == username and st.session_state["passwd"].strip() == password:
...
def authenticate_user():
      if "authenticated" not in st.session_state:
        ...
        col.text_input(label="Username:", value="", key="username", on_change=creds_entered) 
        col.text_input(label="Password:", value="", key="password", type="password", on_change=creds_entered)
        return False
      else:
